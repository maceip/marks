...
test('probe identity and template equality fail closed on both sides', () => {
  ...
  assert.match(releaseRoot, /"nodeToolchain": node_toolchain/u);
  assert.match(releaseRoot, /def validate_node_toolchain_contract\(/u);
  assert.match(releaseRoot, /NODE_TOOLCHAIN_VERSION = "v24\.19\.0"/u);
  assert.match(releaseRoot, /NPM_TOOLCHAIN_VERSION = "11\.17\.0"/u);
  ...
  assert.match(client, /installed helper did not prove the pinned Node 24 build toolchain/u);
  assert.match(client, /\[\[ "\$\(node --version\)" == "v24\.19\.0" \]\]/u);
  assert.match(client, /\[\[ "\$\(npm --version\)" == "11\.17\.0" \]\]/u);
  assert.match(client, /npm ci --engine-strict --strict-allow-scripts/u);
...
test('every untrusted build process has a wall-clock and resource bound', () => {
  ...
  assert.match(releaseRoot, /npm_config_engine_strict=true/u);
  assert.match(releaseRoot, /npm_config_strict_allow_scripts=true/u);
  assert.match(releaseRoot, /--setenv=PATH=\{NODE\.parent\}:\/usr\/local\/sbin/u);
  ...
  assert.match(releaseRoot, /command\.extend\(\[NODE, NPM_CLI, \*arguments\]\)/u);
  assert.doesNotMatch(releaseRoot, /command\.extend\(\["\/usr\/bin\/npm"/u);
  assert.match(releaseRoot, /if os\.geteuid\(\) == 0:\s+validate_node_toolchain\(\)/u);
