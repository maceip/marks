...
export function AgentIconButton({ glyph, tooltip, ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { glyph: GlyphName; tooltip: string }) {
  return <IconButton className="agent-chat-icon-button" label={tooltip} data-tooltip={tooltip} icon={<Glyph name={glyph} size={18} interactive={false} />} {...props} />;
}
...
export function AgentActionButton({ children, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) {
  return <Button className="agent-chat-action" variant="secondary" {...props}>{children}</Button>;
}
