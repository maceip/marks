  ...
  return <div className="ds-matrix" role="list" aria-label={`${kind} state matrix`}>
    {catalogStates.map((state) => <div className={`ds-state state-${state.id}`} role="listitem" key={state.id}>
      ...
      {kind === 'input' ? <input aria-label={`${state.label} input`} value={state.id === 'localized' ? state.label : 'Document title'} disabled={state.id === 'disabled'} readOnly /> :
        <button className={kind === 'pill' ? 'ds-pill' : ''} type="button" disabled={state.id === 'disabled'} aria-pressed={state.id === 'selected'}>
          {state.id === 'loading' && <span className="ds-spinner" aria-hidden="true" />}{state.label}
        </button>}
    </div>)}
  </div>;
}
...
export function DesignSystem({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    return () => { delete root.dataset.density; delete root.dataset.glass; delete root.dataset.motion; delete root.dataset.materialTier; };
  ...
  return <div className="design-system">
    ...
    <main>
      <Section id="foundations" title="Foundations"><h3>Palette & semantic roles</h3><div className="ds-swatches">{palette.map(([name, token]) => <div key={token}><span style={{ background: `var(${token})` }} /><b>{name}</b><code>{token}</code></div>)}</div><h3>Typography</h3><div className="ds-type"><span className="display">Display / 36</span><h1>Heading one / 28</h1><h2>Heading two / 22</h2><p>Body text keeps long-form writing calm and readable.</p><code>Mono / editor and code</code></div><h3>Spacing, size, radius, border, shadow & icons</h3><div className="ds-foundations"><div className="spacing-demo">4<br/>8<br/>12<br/>16<br/>24<br/>32</div>{['sm','default','lg','xl'].map(x => <span className={`radius-${x}`} key={x}>{x}</span>)}<span className="shadow-demo">Elevation</span><div>{Object.entries(icons).slice(0,12).map(([name,path]) => <span className="icon-demo" title={name} key={name}><Icon path={path}/></span>)}</div></div></Section>
      <Section id="components" title="Component state matrices"><p>Every row includes default, simulated interaction, accessibility, stress, and failure states.</p><h3>Buttons & icon buttons</h3><StateMatrix/><div className="ds-icon-buttons">{catalogStates.map(s => <button key={s.id} aria-label={`${s.label} settings`} disabled={s.id === 'disabled'} className={`state-${s.id}`}><Icon path={icons.settings}/></button>)}</div><h3>Pills & status indicators</h3><StateMatrix kind="pill"/><div className="ds-status"><span>● Connected</span><span>◌ Saving</span><span>▲ Attention</span><span>● Offline</span></div><h3>Inputs & tabs</h3><StateMatrix kind="input"/><div role="tablist" className="ds-tabs">{['Write','Preview','Review','History'].map((x,i)=><button role="tab" aria-selected={i===0} key={x}>{x}</button>)}</div><h3>Tooltips, menus, dialogs, sheets & toasts</h3><div className="ds-overlays"><span className="ds-tooltip">Keyboard shortcut <kbd>⌘K</kbd></span><menu><button>Rename</button><button>Duplicate</button><button className="danger">Delete</button></menu><div role="dialog" aria-label="Example dialog"><b>Publish changes?</b><p>Everyone with access will see this version.</p><button>Cancel</button> <button>Publish</button></div><aside>Sheet · Appearance controls</aside><output role="status">✓ Changes saved locally</output></div></Section>
      ...
      <Section id="agent" title="Agent-chat pill states"><div className="ds-agent">{['Idle · Ask Marks','Thinking · Planning edits','Tool · Searching document','Approval needed','Streaming response','Complete · 3 edits','Error · Retry'].map((x,i)=><button className={`agent-${i}`} key={x}><Icon path={icons.sparkles}/>{x}</button>)}</div></Section>
      <Section id="materials" title="Material recipes & live rendering tiers"><div className="ds-materials">{(['cinematic','balanced','foundation','opaque'] as Material[]).map((tier,i)=><article key={tier}><SurfaceMaterial variant={i===0?'hero':i===1?'panel':'floating'} intensity={1-i*.2}/><b>{tier}</b><p>{tier==='opaque'?'Solid raised surface; no transparency.':`${24-i*6}px blur, ${160-i*20}% saturation`}</p></article>)}</div></Section>
