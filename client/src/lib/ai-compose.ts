/**
 * Local composition helpers for the AI ribbon.
 *
 * This is the presentation of AI integration, not a model call. The copy in
 * the UI is honest about that. When a service is wired later it should keep
 * the same modes and apply its result through the same insert path.
 */
...
export function composeLocally({ mode, source, instruction, title }: AiComposeRequest): AiComposeResult {
...
Write here. The AI ribbon can rewrite, shorten, or outline this locally until a model is connected.
`;
}
