...
impl AgentHub {
    pub fn new(
        ...
        let provider = match injected_provider {
            ...
            None if config.provider == AgentProviderKind::OpenAi => {
                Some(Arc::new(openai::OpenAiProvider::from_config(&config)?)
    ...
    pub fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            limits: CapabilityLimits {
                ...
                max_concurrent_runs_per_session: self.config.max_concurrent_runs_per_session,
            },
    ...
    pub(crate) fn load_owned_run(
        ...
        let (
            (_, principal, document, _, created, expires, output, input, output_tokens),
            status,
            rows,
        ) = persisted;
    ...
    fn spawn_run(
        ...
        let task = tokio::spawn(async move {
            ...
            match outcome {
                Ok(Ok(completion)) => {
                    let _ = hub.finalize(&run, RunStatus::Completed, None, completion.usage);
                }
                Ok(Err(error)) if error.code == "cancelled" || run.cancel.is_cancelled() => {
                    let _ = hub.finalize(&run, RunStatus::Cancelled, None, AgentUsage::default());
                }
                Ok(Err(error)) => {
                    let _ = hub.finalize(
                        &run,
    ...
    fn finalize(
        ...
        let (kind, data) = match status {
            ...
            RunStatus::Cancelled => ("run.cancelled", json!({ "status": RunStatus::Cancelled })),
...
impl AgentDriver {
    ...
    pub async fn cancelled(&self) {
        self.run.cancel.cancelled().await;
    }
}
