...
impl AgentProvider for OpenAiProvider {
    ...
    async fn run(
        ...
        loop {
            ...
            let turn = self
                .execute_turn(&request, &input, |delta| driver.emit_text(delta))
                .await?;
...
fn process_frame(
    ...
    let frame = std::str::from_utf8(frame)
        .map_err(|_| ProviderError::new("malformedProviderResponse"))?;
...
mod tests {
    ...
    async fn malformed_upstream_is_normalized_without_echoing_its_body() {
        ...
        let error = provider
            .execute_turn(&request, &[json!({ "role": "user", "content": "hello" })], |_| {
                Ok(())
            })
            .await
            .unwrap_err();
